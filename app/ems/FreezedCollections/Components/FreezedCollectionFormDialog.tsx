...
interface FreezedCollectionFormDialogProps {
  open: boolean;
  onClose: () => void;
  selectedCollection: FreezedCollection | null;
}
...
const FreezedCollectionFormDialog: React.FC<FreezedCollectionFormDialogProps> = ({
  open,
  onClose,
  selectedCollection,
}) => {
  ...
  const { mutateAsync: createFreezedCollection, isPending: isCreating } = useMutation({
    mutationFn: (data: Parameters<typeof api.createFreezedCollection>[0]) =>
      api.createFreezedCollection(data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["freezedCollections"] });
      message.success("تم إضافة السجل بنجاح");
      onClose();
    },
    onError: () => {
      message.error("حدث خطأ أثناء إضافة السجل.");
    },
  });

  const { mutateAsync: updateFreezedCollection, isPending: isUpdating } = useMutation({
    mutationFn: ({ id, data }: { id: number; data: Partial<FreezedCollection> }) =>
      api.updateFreezedCollection(id, data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["freezedCollections"] });
      message.success("تم تحديث السجل بنجاح");
      onClose();
    },
    onError: () => {
      message.error("حدث خطأ أثناء تحديث السجل.");
    },
  });
  ...
  const handleSave = async () => {
    ...
    if (isEditing && selectedCollection?.id) {
      await updateFreezedCollection({ id: selectedCollection.id, data: payload });
    } else {
      await createFreezedCollection(payload);
    }
  };
  ...
  return (
    <FormModal
      open={open}
      onClose={onClose}
      title={isEditing ? "تعديل تجميد/سحب" : "إضافة تجميد/سحب"}
    ...
    >
      ...
      <FormControl fullWidth size="small" required>
        <InputLabel id="freezed-collection-type-label">نوع التحصيل</InputLabel>
        <Select
          labelId="freezed-collection-type-label"
...
export default FreezedCollectionFormDialog;
